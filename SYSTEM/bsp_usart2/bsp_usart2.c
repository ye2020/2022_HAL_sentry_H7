...
//    __HAL_DMA_ENABLE_IT(huart2.hdmatx, DMA_IT_TC);
    ...
    /* 使能串口DMA接收 */
		HAL_UART_Receive_DMA(&huart2, Usart2_Rx, USART2_RX_LEN); // 开启串口DMA接收（记得开启循环模式）
...
void USART2_IRQHandler_callback(UART_HandleTypeDef *huart)
{
	...
	{		
		...
		__HAL_UART_CLEAR_IDLEFLAG(&huart2);		// 清除挂起标准
		num = huart->Instance->ISR; //清除RXNE标志（中断和状态寄存器）
		num = huart->Instance->RDR; //清USART_IT_IDLE标志（接收数据寄存器）
...
//		__HAL_DMA_CLEAR_FLAG(huart2.hdmarx, DMA_FLAG_TCIF1_5); 
		...
		__HAL_DMA_ENABLE(&hdma_usart2_rx);														// 开启DMA（记得呀 ！！）
...
{	
    ...
    if (result != 0 && (huart2.gState == HAL_UART_STATE_READY || huart2.gState == HAL_UART_STATE_BUSY_TX))
    {
